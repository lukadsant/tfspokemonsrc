		...
		if addBall then
			...
			if corpseSkull ~= nil then
				if corpseSkull == SKULL_GREEN then
					sexStr = "Female"
				elseif corpseSkull == SKULL_WHITE then
					sexStr = "Male"
				elseif corpseSkull == SKULL_NONE then
					sexStr = "None"
				else
					sexStr = "Other"
				end
...
-- helper used with addEvent to reliably set a monster's skull after creation
function doSetMonsterSkull(uid, skull)
...
-- alternative helper: find the player's active summon and set its skull
function doSetMonsterSkullByMaster(cid, skull)
	local player = Player(cid)
...
function doSetMonsterNicknameByMaster(cid, nickname)
	local player = Player(cid)
	if not player then return true end
	local summons = player:getSummons()
	if not summons or #summons == 0 then return true end
