		...
		if ($news) {
			...
			if ($view !== "") { // We want to view a specific news post
				...
				if ($si !== false) {
					...
					<table id="news">
						<tr class="yellow">
							<td class="zheadline"><?php echo '[#'.$news[$si]['id'].'] '. getClock($news[$si]['date'], true) .' by <a href="characterprofile.php?name='. $news[$si]['name'] .'">'. $news[$si]['name'] .'</a> - <b>'. TransformToBBCode($news[$si]['title']) .'</b>'; ?></td>
						</tr>
						<tr>
							<td>
